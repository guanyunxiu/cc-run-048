...
export class GAPlanner implements PathPlanner {
  ...
  plan(start: Vec3, goal: Vec3): PlannerOutput {
    ...
    const recordConvergence = (it: number) => {
      let best = Infinity
      let sum = 0
      for (const ind of pop) {
        if (ind.cost < best) best = ind.cost
        sum += ind.cost
      }
      convergence.push({
        iteration: it,
        bestCost: best,
        meanCost: sum / pop.length,
        bestSoFar: Math.min(
          best,
          convergence.length ? convergence[convergence.length - 1].bestSoFar : Infinity
        )
      })
    }
    recordConvergence(0)
    ...
    for (let it = 0; it < tu.gaIterations; it++) {
      ...
      pop.length = 0
      pop.push(...next)
      if (it === tu.gaIterations - 1 || it % Math.max(1, Math.floor(tu.gaIterations / 40)) === 0) {
        recordConvergence(it + 1)
      }
    }
