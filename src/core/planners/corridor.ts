...
export function fitness(
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    const l = dist(a, b)
    length += l
    threat += env.integrateField(a, b, (p) => env.threatIntensity(p))
    nofly += env.integrateField(a, b, (p) => env.noflyPenalty(p))
    altitude += env.integrateField(
      a,
      b,
      (p) => Math.abs(p.y - plan.cruiseAlt) / Math.max(plan.cruiseAlt, 1)
    )
    if (i >= 2) {
      const v1x = a.x - path[i - 2].x
      const v1z = a.z - path[i - 2].z
      const v2x = b.x - a.x
      const v2z = b.z - a.z
      const l1 = Math.hypot(v1x, v1z)
      if (l1 > 1e-6) {
        const c = (v1x * v2x + v1z * v2z) / (l1 * Math.hypot(v2x, v2z))
        smooth += 1 - Math.max(-1, Math.min(1, c))
      }
    }
  ...
  return (
    length * weights.distance +
    threat * weights.threat +
    nofly * weights.nofly +
    altitude * weights.altitude +
    smooth * length * weights.smooth +
    collisionPenalty
  )
}
