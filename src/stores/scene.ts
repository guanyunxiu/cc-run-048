  ...
  actions: {
    ...
    serialize(
      extra: Pick<SerializedScene, 'rawPath' | 'smoothPath' | 'trajectory' | 'stats'> & {
        ...
        viewLayers?: import('@/types').ViewLayerOptions
      }
    ): SerializedScene {
      return {
        version: '3.0.0',
        ...
        fleet: extra.fleet ? JSON.parse(JSON.stringify(extra.fleet)) : undefined,
        viewLayers: extra.viewLayers
          ? JSON.parse(JSON.stringify(extra.viewLayers))
          : undefined,
        ...extra
      }
    },
    ...
    loadScene(data: SerializedScene) {
      ...
      // v2.x 旧存档补全新增权重维度（迭代三）
      this.weights = {
        ...defaultWeights,
        ...data.weights,
        energy: data.weights?.energy ?? defaultWeights.energy,
        dynamics: data.weights?.dynamics ?? defaultWeights.dynamics
      }
      this.replanTriggers = {
        ...defaultReplanTriggers,
        ...data.replanTriggers
