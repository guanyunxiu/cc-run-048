...
/** 读取视图层的规划缓存开关（惰性，避免 store 初始化顺序问题） */
function useViewCacheEnabled(): boolean {
  try {
    return useViewStore().perf.planCache
  } catch {
    return true
  }
}
...
interface SimState {
  ...
  loop: boolean
  /** 迭代三：规划缓存（同参数命中免重算） */
  cache: PlanCache<string, PlanResult>
  cacheHits: number
  cacheMisses: number
}
...
export const useSimStore = defineStore('sim', {
  state: (): SimState => ({
    ...
    plannedLength: 0,
    loop: false,
    cache: new PlanCache<string, PlanResult>(8),
    cacheHits: 0,
    cacheMisses: 0
  }),
  ...
  actions: {
    ...
    advance(dt: number) {
      if (!this.playing || this.trajectory.length < 2) return
      ...
      if (this.onlineReplan) this.handleOnlineReplan()
    },
    ...
    /** 通过 Web Worker 执行规划（并行，不阻塞渲染）；迭代三：规划缓存命中免重算 */
    plan(): Promise<PlanResult> {
      const scene = useSceneStore()

      // 迭代三：规划缓存（与 Worker 请求同口径的键）
      const cacheInput = {
      ...
      const cacheEnabled = useViewCacheEnabled()
      if (cacheEnabled) {
        const key = planCacheKey(cacheInput)
        ...
        if (cached) {
          ...
          this.applyPlanResult(cached, scene.planParams, scene.weights, env)
          this.message = '规划成功（缓存命中）'
          return Promise.resolve(cached)
        }
        this.cacheMisses++
        return this.planViaWorker(scene, key)
      }
      return this.planViaWorker(scene, null)
    },

    /** 单步前进/后退（迭代三：时间轴步进） */
    stepFrame(deltaSec: number) {
      if (this.trajectory.length < 2) return
      this.pause()
      this.seek(this.simTime + deltaSec)
    },
    ...
    /** 实际发送 Worker 请求并在成功后写缓存 */
    planViaWorker(scene: ReturnType<typeof useSceneStore>, cacheKeyStr: string | null): Promise<PlanResult> {
      ...
      return new Promise((resolve) => {
        ...
        worker.onmessage = (ev: MessageEvent<WorkerResponse>) => {
          ...
          if (msg.type === 'plan-done') {
            ...
            if (cacheKeyStr && msg.result.success) this.cache.set(cacheKeyStr, msg.result)
