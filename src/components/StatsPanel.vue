  ...
  <div>
    <div class="section">
      ...
      <div class="stat-grid">
        ...
        <div class="stat-card">
          <div class="k">总能耗 (kJ)</div>
          <div class="v">{{ s?.energyKJ !== undefined ? s.energyKJ.toFixed(1) : '—' }}</div>
        </div>
        <div class="stat-card">
          <div class="k">平滑度 (rad/100m)</div>
          <div class="v">{{ s?.smoothness !== undefined ? s.smoothness.toFixed(3) : '—' }}</div>
        </div>
    ...
    <!-- 迭代三：收敛曲线 -->
    <div v-if="s?.convergence && s.convergence.length > 0" class="section">
      <div class="section-title">算法收敛曲线（{{ s.algo }}）</div>
      <ConvergenceChart :data="s.convergence" />
    </div>
