...
const hasTimeline = computed(
  () => sim.trajectory.length >= 2 || engine.fleetActive
)
...
<template>
  <div class="playback">
    <div class="left">
      <button
        class="primary"
        :disabled="!hasTimeline || sim.status === 'planning'"
        @click="sim.togglePlay()"
        :title="'空格播放/暂停'"
      >
        {{ sim.playing ? '⏸ 暂停' : '▶ 播放' }}
      </button>
      <button :disabled="!hasTimeline" @click="sim.seek(0)">⏮ 回放</button>
      <button
    ...
    <div class="right">
      <span v-if="engine.fleetActive" class="mini fleet-badge">
        🛩 编队 ×{{ engine.fleetRuntimes.filter((r) => r.path.length >= 2).length }}
        · 最近间隔 {{ isFinite(engine.fleetMinSeparation) ? engine.fleetMinSeparation.toFixed(0) + 'm' : '—' }}
      </span>
