...
<template>
  <div>
    ...
    <div class="section">
      <div class="section-title">代价权重（7 维，实时影响航迹）</div>
      <p class="w-tip">航程/威胁/高度/禁飞/平滑 + 能耗 + 动力学违反惩罚</p>
      <NumberSlider label="航程代价" v-model="w.distance" :min="0" :max="10" :step="0.1" :decimals="1" />
      <NumberSlider label="威胁暴露" v-model="w.threat" :min="0" :max="150" :step="1" />
      <NumberSlider label="高度代价" v-model="w.altitude" :min="0" :max="60" :step="0.5" :decimals="1" />
      <NumberSlider label="禁飞惩罚" v-model="w.nofly" :min="0" :max="300" :step="2" />
      <NumberSlider label="平滑代价" v-model="w.smooth" :min="0" :max="3" :step="0.05" :decimals="2" />
      <NumberSlider label="能耗代价" v-model="w.energy" :min="0" :max="200" :step="2" />
      <NumberSlider label="动力学惩罚" v-model="w.dynamics" :min="0" :max="250" :step="2" />
